...
"""  """
# ---------------------------------------------------------------------------
from data.dictionarys import segmentation_extraction_clipped_layers, decimated_layers, segmentation_extraction_layers
...
def do(self, use: str, param: []):
    ...
    for idx, elem in enumerate(decimated_layers.values()):
    ...
    if extraction_semaphor[0] == 0 or segmentation_semaphor[0] == 0:
        for tex, name in zip(textures, segmentation_extraction_clipped_layers.keys()):
            segmentation_extraction_layers[name] = self.plotter.add_mesh(
                mesh=segmentation_extraction_clipped_layers[name], texture=tex,
                name=name,
        ...
        for col, name in zip(colors, segmentation_extraction_clipped_layers.keys()):
            segmentation_extraction_layers[name] = self.plotter.add_mesh(
                mesh=segmentation_extraction_clipped_layers[name], color=col, name=name,
                show_scalar_bar=False, reset_camera=False)
