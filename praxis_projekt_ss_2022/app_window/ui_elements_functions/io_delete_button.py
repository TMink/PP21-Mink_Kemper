...
def do(self):
    found = [val for key, val in interactable_objects_plotted.items() if button_selected[0] == key]
    key_item = '_'
    if found:
        for key, value in interactable_objects_plotted.items():
            if value == found[0]:
                self.plotter.remove_actor(key)
                key_item = key
        interactable_objects_plotted.pop(key_item)
